"""Dev server entrypoint: `PYTHONPATH=src python -m app` from repo root. Binds 0.0.0.0 (override with HOST).

HTTPS: set both SSL_CERTFILE and SSL_KEYFILE (PEM paths). Uvicorn terminates TLS; the app
still sees HTTP semantics. `make run` uses src/scripts/dev_tls_cert.sh; use `make run-http` for plain HTTP.
"""
