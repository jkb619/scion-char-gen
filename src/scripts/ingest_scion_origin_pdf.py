#!/usr/bin/env python3
"""
Extract plain text from Scion: Origin (Revised) PDF for grep / Knack verification.

Many Mortal / pre-Visitation Knacks in knacks.json cite Origin alongside Pandora’s Box.
"""
...
from scion_books_dir import find_scion_origin_revised_pdf

DEFAULT_OUT = SRC / "data" / "_extracted" / "scion_origin_revised.txt"
DEFAULT_PDF_WSL = Path("/mnt/c/Users/John/Desktop/Scion/books/Scion_Origin_(Revised_Download).pdf")


def main() -> int:
    parser = argparse.ArgumentParser(description="Extract text from Scion: Origin Revised PDF")
    parser.add_argument(
        "pdf",
        type=Path,
        nargs="?",
    ...
    if pdf is None or not str(pdf).strip():
        pdf = find_scion_origin_revised_pdf(books)
        if pdf is None and DEFAULT_PDF_WSL.is_file():
            pdf = DEFAULT_PDF_WSL
    if pdf is None or not pdf.is_file():
        print(
            "Pass the path to Scion_Origin_(Revised_Download).pdf or place it under books/.",
            file=sys.stderr,
        )
