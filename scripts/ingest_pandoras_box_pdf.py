#!/usr/bin/env python3
"""
Extract plain text from SCION Pandora's Box (Revised) for Boon ladder extraction.

This PDF is the app’s **primary** mechanical reference for Boons, standard Purviews, Knacks, and Birthrights.

Usage:
  pip install pypdf
  python3 scripts/ingest_pandoras_box_pdf.py /path/to/SCION_Pandoras_Box_(Revised_Download).pdf
  python3 scripts/ingest_pandoras_box_pdf.py   # uses default path when present
"""
...
DEFAULT_OUT = ROOT / "data" / "_extracted" / "pandoras_box.txt"
DEFAULT_PDF_WSL = Path("/mnt/c/Users/John/Desktop/Scion/books/SCION_Pandoras_Box_(Revised_Download).pdf")


def main() -> int:
    parser = argparse.ArgumentParser(description="Extract text from Pandora's Box PDF")
    parser.add_argument(
        "pdf",
        type=Path,
        nargs="?",
